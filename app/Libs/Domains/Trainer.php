...
readonly class Trainer
{
     ...
     * 重賞調教師かどうかを判定
     */
    public function isGradeTrainer(): bool
    {
        return $this->getGradeWinsCount() > 0;
    }

    /**
     * トップトレーナーかどうかを判定（G1勝利あり）
     */
    public function isTopTrainer(): bool
    {
        return $this->getG1Wins() > 0;
    }

    /**
     * リーディングトレーナーかどうかを判定（年間30勝以上）
     */
    public function isLeadingTrainer(): bool
    {
        $latestStat = $this->getLatestStat();
        return $latestStat !== null && $latestStat->wins >= 30;
    }
